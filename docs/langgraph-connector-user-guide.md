# Conduit LangGraph Connector — User Guide

This guide explains how to connect Conduit to a **LangGraph** agent using the **LangGraph connector** (`connection_type: "langgraph"`). The connector uses the [LangGraph SDK](https://github.com/langchain-ai/langgraph-sdk) to talk to LangGraph Cloud or a self-hosted LangGraph API.
...
- Conduit uses the **assistant → thread → run** pattern: each conversation has a **thread**, and each user message triggers a **run** for the configured **assistant** on that thread.
- The connector **streams** the assistant’s reply and supports **interrupts**: when your graph calls `interrupt(...)`, the interrupt payload is sent to the client; when the user replies or clicks a button, Conduit **resumes** the run with the new input.
...
Create a LangGraph agent in the Conduit admin UI or via the API:
...
\* If you use the Conduit UI to pick an assistant from a list, `graph_id` is needed so Conduit can fetch assistants.  
...
| `metadata` | `dict` | Conduit metadata: `user` (e.g. `user_id`, `username`, `ad_group`), `conversation` (e.g. `conversation_id`), `project`. Also includes **`frontend`**: the dynamic panel state (selections, text input values, etc.) from the last user action. |
| `attachments` | `list` | URI-based attachments (e.g. S3). |
| `context` | `dict` | Client context passed through from the chat request (e.g. timezone, locale, or the same panel state). Conduit passes the frontend panel state here as well as in `metadata.frontend`. |
...
- **Panel state (e.g. table selections, text field values):** from `metadata.frontend` or `context` (depending on how Conduit is configured; the chat service puts panel state in both).

---

## Thread Lifecycle

- **First message in a conversation:** Conduit has no thread ID. The connector **creates a new thread** via the LangGraph API and then starts a run on it. Conduit stores the returned `thread_id` on the conversation.
- **Later messages:** Conduit passes the stored `thread_id` into the connector so the run is on the same thread (same conversation).
- **Interrupt:** When your graph calls `interrupt(...)`, the connector streams the interrupt content to the user. Conduit marks that conversation as “interrupted” so the **next** request for that conversation is sent as a **resume** with the same thread ID and the new user input (message + panel state).

So you do not create or delete threads in your graph; the connector and Conduit manage creation and persistence.

---

## Assistants and graph_id

- **Listing assistants:** If the agent has a `graph_id` (on the agent or in `extras`), Conduit can call the LangGraph API to **search assistants** for that graph. This is used by the UI to show a list of assistants (e.g. to set `assistant_id`).
...
2. **User action:** The user types a reply or uses a dynamic element (e.g. button). Conduit sends the next request with the new message and updated panel state.
...
Run the examples with the LangGraph CLI or your preferred runner, then in Conduit create a LangGraph agent with the appropriate `endpoint`, `graph_id`, and `assistant_id` (or name) so Conduit uses the same graph and assistant.
