"""
LangGraph example: all Conduit dynamic UI elements and file download.
...
Conduit's LangGraph agent at it. Flow:
...
def welcome(state: State) -> dict:
    ...
    payload = AgentResponse(
        content="Welcome to the Conduit elements demo. Below are sample **stats** and a button.",
        elements=[
            {
                "type": "stats",
                "id": "welcome_metrics",
...
def file_download_step(state: State) -> dict:
    ...
    sample_content = b"Conduit elements demo - file download.\n\nThis file was generated by the LangGraph example."
...
def goodbye_step(state: State) -> dict:
    ...
    payload = AgentResponse(
        content="Thanks for trying the Conduit elements demo. The panel is now cleared.",
        elements=[],
    ).model_dump(exclude_none=True)
    interrupt(payload)
    return {"step": 5}
